...
contract VolcanoInsuranceOwnerMock is ChainlinkClient {
    ...
    function BuyerCreateMockPolicy(int inputLat, int inputLong, uint yearS, uint monthS, uint dayS) public payable {
        OpenWEItoInsure -= (1*(10**18));
        LockedWEItoPolicies += (1*(10**18));
        policies[msg.sender] = policy(inputLat, inputLong, yearS, monthS, dayS, 1);
    }

    function setMockPresentDates(uint _YearPresent, uint _MonthPresent, uint _DayPresent) public {
    ...
    function OwnerSelfDestructClaimETH() public contractOwnerCheck {
        require(address(this).balance > (LockedWEItoPolicies+OpenWEItoInsure), 'No self destruct detected (address(this).balance == (LockedWEItoPolicies+OpenWEItoInsure))'); 
